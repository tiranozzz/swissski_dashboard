...
from queries import get_hist_data, get_test_files, get_athlete_data, read_excel
...
def train_models(file_df, model_dict, filename):
...
with col1:
    ...
    for f in files:
        ...
        file_df = read_excel(file=files_dict["path"] + "\\" + files_dict["filename"], supported_attributes=config["test_attributes"], config=config)
        files_dict["df"] = file_df
        # Train models
        files_dict["model_dict"] = train_models(file_df=file_df, model_dict=model_dict, filename=f)
        ...
        text_expander = f":page_facing_up: {f} ({len(file_df)} rows). Model results: {green_cnt} :large_green_square: | {yellow_cnt} :large_yellow_square: | {red_cnt} :large_red_square:"
        if error_cnt > 0:
